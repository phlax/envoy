...
//
// Replaces the `certs.sh` scripts that used to shell out to the OpenSSL CLI
// (and, in one case, to `docker run ... faketime`). Everything is produced
// in-process with BoringSSL from a declarative spec file so that fixtures are
// reproducible, year-stamped and free of external tooling.
//
// Usage:
//   gen --spec <spec-file> --in-dir <dir> --out-dir <dir> --year <YYYY>
//
// See tools/testcerts/README.md for the spec file format.
...
private:
  ...
  // order. The string type mirrors OpenSSL's ASN1_STRING_TABLE defaults.
  ...
  void makeCert(const Section& section) {
    ...
    // a file of its own (the shell scripts used to delete these).
    if (out_name != "none") {
  ...
  void writeInfoHeader(const std::string& fixture_name, const X509& cert,
    ...
    out += "// NOLINT(namespace-envoy)\n";
    out += "// This file is auto-generated by //tools/testcerts:gen.\n";
